      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-10 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {refundSteps.map((item, idx) => (
            <div
              key={idx}
              className="group bg-white dark:bg-slate-900 p-8 rounded-3xl shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-slate-800 hover:-translate-y-2 transition-transform duration-500"
            >
        ...
        <div className="mt-20 grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="p-8 rounded-3xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800">
            <div className="flex items-center gap-4 mb-6">
              ...
              <h3 className="text-2xl font-bold text-slate-900 dark:text-white">Non-Refundable Items</h3>
            </div>
            <ul className="space-y-4 text-slate-600 dark:text-slate-300">
              <li className="flex items-start gap-3">
                <span className="w-1.5 h-1.5 rounded-full bg-rose-500 mt-2 shrink-0" />
                <span>Service fees for completed visits where care was delivered as agreed.</span>
              </li>
              <li className="flex items-start gap-3">
                <span className="w-1.5 h-1.5 rounded-full bg-rose-500 mt-2 shrink-0" />
                <span>Cancellations made less than 4 hours before the scheduled appointment time.</span>
              </li>
              <li className="flex items-start gap-3">
                <span className="w-1.5 h-1.5 rounded-full bg-rose-500 mt-2 shrink-0" />
                <span>Administrative processing fees and platform charges.</span>
              </li>
            </ul>
          </div>

          <div className="p-8 rounded-3xl bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-100 dark:border-emerald-900/30">
            <div className="flex items-center gap-4 mb-6">
              ...
              <h3 className="text-2xl font-bold text-slate-900 dark:text-white">Need Assistance?</h3>
            </div>
            <p className="text-slate-600 dark:text-slate-300 mb-6 leading-relaxed">
              If you believe you have been charged incorrectly or experienced subpar service, please contact our support team immediately. We are here to help resolve any issues fairly.
            </p>
            <button className="px-6 py-3 bg-emerald-600 text-white font-bold rounded-xl hover:bg-emerald-700 transition-colors shadow-lg shadow-emerald-500/20">
              Contact Support
            </button>
          </div>
        </div>
