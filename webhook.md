...
async function handleCheckoutSessionCompleted(session) {
  try {
    ...
    if (existingPayment) {
      console.log(
        `[Webhook] Payment already processed: ${session.payment_intent}`,
      );
    ...
    if (!bookingResult.insertedId) {
      console.error(
        `[Webhook] Failed to create booking for session: ${session.id}`,
      );
...
async function handlePaymentIntentSucceeded(paymentIntent) {
  try {
    ...
    console.log(
      `[Webhook] Waiting for checkout.session.completed for: ${paymentIntent.id}`,
    );
...
async function handlePaymentIntentFailed(paymentIntent) {
  try {
    ...
    console.log(
      `[Webhook] Failure reason: ${paymentIntent.last_payment_error?.message || "Unknown"}`,
    );
...
_Generated by Antigravity AI Assistant - Production Mindset Series_
